...
(* CHANGE: Add a case for Shift expressions *)
      | Shift(dx,dy,e) =>
          (case eval_prog(e,env) of
              ...
              | _ => raise Impossible("haha"))

(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)
fun preprocess_prog e = 
  let 
    fun is_leftlower (x1,y1) (x2,y2) =
      x1<x2 orelse (real_close(x1,x2) andalso (y1<y2+epsilon))
  in
    case e of
         LineSegment(x1,y1,x2,y2) => 
           if not (is_leftlower (x1,y1) (x2,y2))
           then 
             preprocess_prog(LineSegment(x2,y2,x1,y1))
           else
